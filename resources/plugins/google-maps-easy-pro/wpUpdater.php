	...
	class wpUpdaterGmp {
		...
		public function checkForPluginUpdate($checkedData) {
			...
			if (empty($request_args['version']))
				return $checkedData;
			if(class_exists('frameGmp') && frameGmp::_()->getModule('license') && !frameGmp::_()->getModule('license')->getModel()->isExpired()) {
		...
		public function prepareRequest($action, $args) {
			...
			return array(
				...
				'user-agent' => $this->_getUserAgentHash(). '/' . $wp_version . '; ' . GMP_SITE_URL. ';'. $this->getIP()
			);
		}
